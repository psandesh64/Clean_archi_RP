...
const ProductType = () => {
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            const response = await productRepository.createType(formData);
    ...
    const handleDelete = async (id:number) => {
        try {
            const response = await productRepository.deleteType(id);
            if (response.message) {
    ...
    const handleEdit = async () => {
        try {
            const response = await productRepository.updateType({id:editId,name:name,isActive:isActive});
            if (response.message) {
                toast.success(response.message, {
                    position: "top-right",
                    autoClose: 3000,
    ...
    useEffect(()=>{
        const fetchData = async () => {
            try {
              const resp = await productRepository.getManyTypes(); // Wait for the promise to resolve
              setProducts(resp.productTypes); // Print the resolved response
            } catch (error) {
    ...
    <div>
        <fieldset className="my-5 relative border border-[#e0e2e7] shadow p-10 rounded grid gap-x-[48px] gap-y-[40px]
            ...
            <div className="space-y-2">
                <p className="text-[#16141b] text-xs font-semibold font-['Open Sans']">Product Type Name</p>
        ...
        <div className="w-full mt-5 shadow border border-[#e0e2e7]">
            ...
            <div className="flex flex-wrap gap-4 items-center justify-between px-6 py-[18px]">
                ...
                <p className="text-[#333843] text-lg font-medium font-inter text-nowrap">Product Type List</p>
                ...
                <table className='w-full'>
                    <thead>
                        <tr className='px-[22px] py-[18px] h-[56px] bg-[#f9f9fc]'>
                            <th className="pl-4 text-[#333843] text-left text-sm font-medium font-inter">Product Type Name</th>
